...
def log(msg: str, *, mode: str | None = None, run: str | None = None, ocr: str | None = None, src: str | None = None):
    """Print uniforme sur stderr avec préfixes optionnels."""
    prefix = []
    if mode:
...
# ------------------------------------------------------------
# Configuration OCR (source de vérité : USE_OCR)
# ------------------------------------------------------------

def set_use_ocr(enabled: bool) -> None:
    """
    Définit USE_OCR dans l'environnement pour ce processus.
    True  -> USE_OCR=1, False -> USE_OCR=0
    """
    os.environ["USE_OCR"] = "1" if enabled else "0"


def use_ocr() -> bool:
    """Retourne l'état OCR depuis l'environnement (USE_OCR, défaut=1)."""
...
# Champs potentiels pour extraire un nom de produit depuis le JSON LLM
PRODUCT_KEYS = (
    "product_name",
    "product",
    "productName",
...
def _normalize_product_token(value) -> str:
    """Nettoie un nom de produit pour comparaison (minuscule + espaces compactés)."""
    if value is None:
        return ""
...
def _iter_product_values(obj: dict):
    """Itère sur les valeurs produit possibles d'un dict LLM."""
...
def _extract_product_set(out: str) -> set[str]:
    """Construit un set de produits normalisés depuis un JSONL de sortie LLM."""
...
def _extract_patent_set(out: str) -> set[str]:
    """Construit un set de brevets normalisés depuis un JSONL de sortie LLM."""
    patents: set[str] = set()
...
def _log_ocr_diff(base_set: set[str], ocr_set: set[str], *, mode: str, label: str):
    """Logue les différences entre run A (sans OCR) et run B (avec OCR)."""
    additions = sorted(ocr_set - base_set)
    removed = sorted(base_set - ocr_set)
    log(
        f"[OCR-CHECK][{label}] A (sans OCR)={len(base_set)} | B (avec OCR)={len(ocr_set)} | +OCR={len(additions)} | -OCR={len(removed)}",
        mode=mode,
    )
    if additions or removed:
        if additions:
            log(f"[WARN][OCR][{label}] Ajouts OCR (B - A): {', '.join(additions)}", mode=mode)
        if removed:
            log(f"[WARN][OCR][{label}] Retirés quand OCR actif (A - B): {', '.join(removed)}", mode=mode)
    else:
        log(f"[OCR-CHECK][{label}] Pas de différence entre les ensembles finaux (A vs B)", mode=mode)
    return additions, removed


def _start_label(url: str) -> str:
    """Message START avec URL uniquement si LOG_URL_START=1."""
...
def _should_run_ocr(url: str) -> bool:
    """Décide si l'OCR doit être tenté (PDF ou HTML rendable)."""
    if not use_ocr():
        return False
    if not url:
        return False
...
def _log_ocr_html_diff(native_pages: list[str], ocr_pages: list[str], url: str, *, mode: str | None = None, run: str | None = None, ocr_state: str | None = None, src: str | None = None) -> None:
    """
    Log un warning si l'OCR HTML diverge fortement du texte natif.
    S'applique uniquement aux sources HTML (pas PDF).
    """
    if ocr_state != "on":
        return
    ...
    if ratio < 0.98:
        log(f"[WARN][OCR-HTML] Divergence texte natif vs OCR (similarité={ratio:.2f}, natif={len(native)} chars, ocr={len(ocr)} chars)", mode=mode, run=run, ocr=ocr_state, src=src)
...
async def _run_ocr_task(url: str) -> list[str]:
    """OCR asynchrone, lancé en parallèle (PDF ou HTML capturé en PNG)."""
    try:
        if _looks_like_pdf(url):
...
async def _extract_products_once(url: str, enable_ocr: bool, run_label: str) -> tuple[str, set[str]]:
    """Exécute un run produit (avec ou sans OCR) et retourne (output, set produits)."""
        ...
        if enable_ocr and ocr_pages:
            try:
                ...
                if audit:
                    ...
                    if new_items:
                        out = "\n".join([out, *new_items])
                        log(f"[VERIFY products] +{len(new_items)} produits ajoutés depuis OCR", mode=mode, run=run_label, ocr="on", src=src)
            except Exception as e:
                log(f"[VERIFY products] erreur audit : {e}", mode=mode, run=run_label, ocr="on", src=src)
        elif enable_ocr:
            log("OCR demandé mais aucune page OCR (capture/ocr vide)", mode=mode, run=run_label, ocr="on", src=src)
...
async def analyse_url_products(url: str) -> str:
    """Extraction des produits avec comparaison OCR (A sans OCR, B avec OCR si activé)."""
    ...
    if not use_ocr():
        log("[OCR] USE_OCR=0 → comparaison OCR désactivée, sortie sans OCR", mode="products")
        return out_no_ocr
    ...
    # Par défaut, on renvoie le résultat avec OCR (run B)
    return out_with_ocr
...
async def _extract_patents_once(url: str, enable_ocr: bool, run_label: str) -> tuple[str, List[str]]:
    """
    Exécute l'extraction brevets complète pour un mode OCR donné.
    Retourne (output_jsonl, liste_de_brevets_normalisés).
    """
    src = "pdf" if _looks_like_pdf(url) else "html"
    mode = "patents"
    start = time.perf_counter()
            ...
            try:
                ...
                if audit:
                    ...
                    if new_items:
                        ...
                        log(f"[VERIFY] +{len(new_items)} brevets via audit OCR : {', '.join(new_numbers)}", mode=mode, run=run_label, ocr="on", src=src)
            except Exception as e:
                log(f"[VERIFY] erreur audit : {e}", mode=mode, run=run_label, ocr="on", src=src)
        elif enable_ocr:
            log("OCR demandé mais aucune page OCR (capture/ocr vide)", mode=mode, run=run_label, ocr="on", src=src)
...
async def analyse_url_patents(url: str) -> str:
    """
    Extraction des brevets avec comparaison OCR :
    - Run A : sans OCR
    - Run B : avec OCR (uniquement si USE_OCR=1)
    Compare les ensembles finaux normalisés (pas les étapes intermédiaires).
    """
    ...
    if not run_ocr:
        log("[OCR] USE_OCR=0 → comparaison OCR désactivée, sortie sans OCR")
        return out_no_ocr
    ...
    log(f"[OCR-CHECK] A (sans OCR)={len(base_set)} | B (avec OCR)={len(ocr_set)} | +OCR={len(additions)} | -OCR={len(removed)}", mode="patents")
    if additions or removed:
        if additions:
            log(f"[WARN][OCR] Ajouts OCR (B - A): {', '.join(additions)}", mode="patents")
        if removed:
            log(f"[WARN][OCR] Retirés quand OCR actif (A - B): {', '.join(removed)}", mode="patents")
    else:
        log("[OCR-CHECK] Pas de différence entre les ensembles finaux (A vs B)", mode="patents")
...
async def _extract_audit_once(url: str, enable_ocr: bool, run_label: str) -> tuple[str, set[str]]:
    """Run audit OCR (avec/sans OCR) et retourne (audit_jsonl, set normalisé)."""
    src = "pdf" if _looks_like_pdf(url) else "html"
    mode = "audit"
    start = time.perf_counter()
    with _temporary_ocr_env(enable_ocr):
        ...
        if audit:
            log(f"[AUDIT] {len(audit.splitlines())} éléments détectés", mode=mode, run=run_label, ocr="on" if enable_ocr else "off", src=src)
        elif enable_ocr:
            log("OCR demandé mais aucune page OCR (capture/ocr vide)", mode=mode, run=run_label, ocr="on", src=src)
...
async def analyse_url_audit(url: str) -> str:
    """Compare produits et brevets extraits vs texte OCR (double run A/B)."""
    log("[MODE] Audit OCR")

    audit_no_ocr, set_no_ocr = await _extract_audit_once(url, enable_ocr=False, run_label="A")
    if not use_ocr():
        log("[OCR] USE_OCR=0 → comparaison OCR désactivée, sortie sans OCR", mode="audit")
        return audit_no_ocr
...
async def _extract_columns_once(url: str, enable_ocr: bool, run_label: str) -> tuple[str, set[str], set[str]]:
    """Pipeline complet (avec/sans OCR) → retourne (output, set produits, set brevets)."""
    src = "pdf" if _looks_like_pdf(url) else "html"
    mode = "full"
    start = time.perf_counter()
    with _temporary_ocr_env(enable_ocr):
        ...
        if not document_text:
            ...
            log("Aucun texte extrait", mode=mode, run=run_label, ocr="on" if enable_ocr else "off", src=src)
            return "", set(), set()
        ...
        if enable_ocr and ocr_pages:
            ...
            if audit:
                ...
                if audit_added_products or audit_added_patents:
                    products_jsonl = to_jsonl(all_products)
                    patents_jsonl = _normalize_llm_patent_lines(to_jsonl(all_patents))
                    log(f"[AUDIT] +{len(audit_added_products)} produits / +{len(audit_added_patents)} brevets ajoutés via OCR", mode=mode, run=run_label, ocr="on", src=src)
        elif enable_ocr:
            log("OCR demandé mais aucune page OCR (capture/ocr vide)", mode=mode, run=run_label, ocr="on", src=src)
...
async def analyse_url_columns(url: str) -> str:
    """Pipeline complet avec comparaison OCR (run A sans OCR, run B avec OCR)."""
    ...
    if not use_ocr():
        log("[OCR] USE_OCR=0 → comparaison OCR désactivée, sortie sans OCR", mode="full")
        return out_no_ocr
