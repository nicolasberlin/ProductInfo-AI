...
from agent.domain.evaluation.normalization import normalize_prod, normalize_pat
...
# ---------- Parsing in-memory LLM output ----------
def pairs_from_result(result: Union[str, list]) -> Set[Pair]:
    """
    Accepts:
      - str NDJSON (one JSON object per line)
      - str JSON (list of objects)
      - list[dict] already parsed
    Returns set of (product, patent)
    """
    items: List[dict] = []
    if isinstance(result, list):
        ...
        raise TypeError("Unsupported LLM result type")
